...
def run_random_eye_tracking_test():
    ...
    result_file = f"random_eye_tracking_result_{timestamp}.json"
    csv_file = f"eye_tracking_data_{timestamp}.csv"
    ...
    if csv_data:
        ...
        print(f"📊 CSV 데이터가 {csv_file}에 저장되었습니다.")
        print(f"   - 총 {len(csv_data)}개 프레임 데이터")
        print(f"   - {len([row for row in csv_data if row['reaction_detected']])}개 반응 시간 기록")
    
    print(f"💾 JSON 결과가 {result_file}에 저장되었습니다.")
