...
def detect_blinks():
    ...
    # 데이터 수집용 리스트
    ear_data = []  # EAR 데이터와 시간을 저장할 리스트
    blink_timestamps = []  # 깜빡임 발생 시간을 저장할 리스트
        ...
        if results.multi_face_landmarks:
            ...
            if not is_measuring:
                ...
                if remaining_time > 0:
                    ...
                    # 결과 자동 저장
                    save_results(blink_counter, ear_data, blink_timestamps)
...
def save_results(blink_count, ear_data=None, blink_timestamps=None):
    """결과를 CSV 파일로 저장"""
    import os
    import json
    ...
    # Data_V2/blinking_test_data/2025-08 형태로 폴더 경로 생성
    data_folder = f"Data_V2/blinking_test_data/{year_month}"
    os.makedirs(data_folder, exist_ok=True)
    
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = os.path.join(data_folder, f"blink_test_{timestamp}.csv")
    ...
    # 추가로 JSON 파일로도 저장 (더 자세한 데이터용)
    json_filename = os.path.join(data_folder, f"blink_test_{timestamp}.json")
    
    detailed_data = {
        'timestamp': timestamp,
        'date': datetime.now().strftime("%Y-%m-%d"),
        'time': datetime.now().strftime("%H:%M:%S"),
        'blink_count_30s': blink_count,
        'blinks_per_minute': blinks_per_minute,
        'status': status,
        'notes': '정상: 15-20회/분, 파킨슨병: 5-10회/분',
        'test_duration': '30초',
        'created_at': datetime.now().isoformat(),
        'baseline_left_ear': None,
        'baseline_right_ear': None,
        'ear_data': ear_data if ear_data else [],
        'blink_timestamps': blink_timestamps if blink_timestamps else []
    }
    
    with open(json_filename, 'w', encoding='utf-8') as jsonfile:
        json.dump(detailed_data, jsonfile, ensure_ascii=False, indent=2)
    
    print(f"📄 상세 데이터가 {json_filename}에도 저장되었습니다!")
    
    # EAR 데이터를 별도 CSV 파일로도 저장 (시계열 분석용)
    if ear_data:
        ear_csv_filename = os.path.join(data_folder, f"ear_data_{timestamp}.csv")
        with open(ear_csv_filename, 'w', newline='', encoding='utf-8') as ear_csvfile:
            ear_fieldnames = ['timestamp', 'elapsed_time', 'left_ear', 'right_ear', 'eyes_closed']
            ear_writer = csv.DictWriter(ear_csvfile, fieldnames=ear_fieldnames)
            ear_writer.writeheader()
            for data_point in ear_data:
                ear_writer.writerow(data_point)
        print(f"📊 EAR 시계열 데이터가 {ear_csv_filename}에 저장되었습니다!")
    
    # 깜빡임 타임스탬프를 별도 CSV 파일로도 저장
    if blink_timestamps:
        blink_csv_filename = os.path.join(data_folder, f"blink_timestamps_{timestamp}.csv")
        with open(blink_csv_filename, 'w', newline='', encoding='utf-8') as blink_csvfile:
            blink_fieldnames = ['blink_number', 'timestamp', 'elapsed_time', 'left_ear', 'right_ear']
            blink_writer = csv.DictWriter(blink_csvfile, fieldnames=blink_fieldnames)
            blink_writer.writeheader()
            for blink_data in blink_timestamps:
                blink_writer.writerow(blink_data)
        print(f"👁️ 깜빡임 타임스탬프 데이터가 {blink_csv_filename}에 저장되었습니다!")
    
    # 저장된 폴더 정보 출력
    print(f"📁 데이터가 {data_folder} 폴더에 저장되었습니다!")
